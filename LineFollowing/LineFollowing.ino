...
void loop()
{
  DDRD |= B11110000;                         // Set direction of Arduino pins D4-D13 as OUTPUT
  PORTD |= B11110000;                        // Set level of Arduino pins D4-D13 to HIGH
  delayMicroseconds(230);                    // Short delay to allow capacitor charge in QTI module
  DDRD &= B00001111;                         // Set direction of pins D4-D13 as INPUT
  PORTD &= B00001111;                        // Set level of pins D4-D13 to LOW
  delayMicroseconds(230);                    // Short delay
  int pins = PIND;                           // Get values of pins D0-D13
  pins >>= 4;                                // Drop off first four bits of the port; keep only pins D4-D13
  
  Serial.println(pins, BIN);                 // Display result of D4-D13 pins in Serial Monitor
